...
- **LESSON PROGRESSION BUG FIX**: Fixed critical bug where completing a lesson would cause the next lesson to start at the wrong step. Added useEffect in lesson-player.tsx to reset currentStep and userChoices state when lessonId changes. This ensures each lesson always starts at step 0 with clean state.
- **AI COURSE GENERATOR RESTRUCTURED FOR THEORY**: Completely redesigned OpenAI GPT-4o prompt to create TEXT-BASED theory lessons instead of practical help desk scenarios. No audio, video, or multimedia content references. New structure focuses on educational concepts with: (1) Context/background instead of help desk tickets, (2) Learning points with explanations instead of troubleshooting steps, (3) Summary with key takeaways instead of root cause analysis. System message explicitly prohibits audio/video references. Content is now purely theoretical and educational, perfect for text-based learning platform.
- **ADMIN CREDENTIALS UPDATED**: Changed admin email from overload@ to overlord@tryrebooto.com (password remains A5dzPbRmggEe)
- **RENDER DEPLOYMENT GUIDE**: Created comprehensive RENDER_DEPLOYMENT_GUIDE.md with step-by-step instructions for deploying Rebooto on Render.com. Covers PostgreSQL database setup, web service deployment, environment variable configuration, database migrations, email integration (Azure AD OAuth2), troubleshooting, monitoring, backups, performance optimization, and security checklist. Includes render.yaml template, custom domain setup, and cost breakdown.
- **PORTABLE OAUTH2 EMAIL SUPPORT**: Enhanced email system to support both Replit Outlook connector AND standard Azure AD OAuth2 for self-hosting. Auto-detects authentication method: if AZURE_CLIENT_ID/AZURE_CLIENT_SECRET/AZURE_TENANT_ID env vars exist, uses OAuth2 client credentials flow; otherwise uses Replit connector. Supports shared mailbox via SHARED_MAILBOX_EMAIL env var. Token caching with auto-refresh. Comprehensive setup guide in AZURE_AD_EMAIL_SETUP.md. Application now fully portable - can be deployed anywhere (Docker, Kubernetes, AWS, Azure, Heroku, Render, etc.) without code changes.
- **MICROSOFT OUTLOOK INTEGRATION**: Integrated Microsoft Outlook connector (connection:conn_outlook_01K8VQET4QJXZY4HRCDAZ4JSRC) for sending emails through Microsoft tenant. Created server/outlook.ts with getUncachableOutlookClient() and sendEmail() functions. Password reset emails now sent via Outlook with branded HTML template including reset link, user's first name, and 1-hour expiry notice. Email includes inline styles for teal/emerald gradient branding. Secure error handling without token exposure in logs.
...
**Email Integration**: Dual-mode email system - Replit Outlook connector (when on Replit) OR Azure AD OAuth2 client credentials flow (when self-hosted). Supports shared mailbox configuration. Auto-detects authentication method via environment variables.
...
- **Lesson Player** (/app/courses/:courseId/lessons/:lessonId): Interactive lesson player supporting scenario-based content (problem/steps/solution format). Features step navigation, progress bar, XP rewards on completion, and automatic navigation to next lesson. Uses memoized content transformation for optimal performance. State management includes useEffect to reset currentStep and userChoices when navigating between lessons, preventing state bleed.
...
- **Course Creator** (/admin/courses): AI-powered course generation tool using OpenAI GPT-4o. Two-step workflow: (1) Generate draft with AI - creates 3-5 lessons with comprehensive theory-based educational content (no audio/video), stored temporarily in React state. (2) Publish course - permanently saves course and lessons to database. Features include course title/description/difficulty inputs, AI generation button, lesson editing capabilities, and publish functionality. Redirects to admin dashboard after successful publish. Content structure: context/learning points/summary format focused on IT concepts and theory.
...
- **Admin Credentials**: overlord@tryrebooto.com / A5dzPbRmggEe (for testing - may be changed during password reset tests)
